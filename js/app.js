...
var app = angular.module('dinamo', ['ionic', 'dinamo.controllers', 'dinamo.directives', 'dinamo.filters', 'dinamo.services', 'tabSlideBox', 'ionic-datepicker', 'pascalprecht.translate', 'ngCookies'])
...
.run(
  ...
  function($ionicPlatform, $ionicScrollDelegate, $rootScope, $interval, $sce, $state, $ionicPopup, $timeout, $cookies, Auth, $translate, baseURL, apiURL) {


    $rootScope.footerlink = false;
    ...
    $rootScope.sportsURL = {
      ...
      'ru' : {
        ...
        'live'    : 'https://sports2.sportbdb.com/ru/Ğ»Ğ°Ğ¹Ğ²-ÑĞ¿Ğ¾Ñ€Ñ‚/?currentSession=',
    ...
    $rootScope.getQueryParams = function (qs) {
      qs = qs.split('+').join(' ');
    ...
    $rootScope.getSportLinks = function(){
      $rootScope.getParams = $rootScope.getQueryParams(window.location.href.replace('https://m.' + domain + '/#/sport/bet', ''));

      $rootScope.sports = {
        main    : $sce.trustAsResourceUrl(apiURL + '/mobile/sport/redirect?type=main&lang=' + $translate.use() + '&t=' + Date.now() + '&path=' + $rootScope.getParams.path + '&basePath=' + 'https://m.' + domain + '/#/sport/bet?path='),
        live    : $sce.trustAsResourceUrl(apiURL + '/mobile/sport/redirect?type=live&lang=' + $translate.use() + '&t=' + Date.now() + '&path=' + $rootScope.getParams.path + '&basePath=' + 'https://m.' + domain + '/#/sport/bet?path='),
        eSport  : $sce.trustAsResourceUrl(apiURL + '/mobile/sport/redirect?type=esport&lang=' + $translate.use() + '&t=' + Date.now() + '&path=' + $rootScope.getParams.path + '&basePath=' + 'https://m.' + domain + '/#/sport/bet?path='),
        virtual : $sce.trustAsResourceUrl(apiURL + '/mobile/sport/redirect?type=virtual&lang=' + $translate.use() + '&t=' + Date.now() + '&path=' + $rootScope.getParams.path + '&basePath=' + 'https://m.' + domain + '/#/sport/bet?path='),
        history : $sce.trustAsResourceUrl(apiURL + '/mobile/sport/redirect?type=history&lang=' + $translate.use() + '&t=' + Date.now() + '&path=' + $rootScope.getParams.path + '&basePath=' + 'https://m.' + domain + '/#/sport/bet?path=')
      };
    ...
    $rootScope.login  = function() {
      ...
      Auth.login(null, function(data) {
        if(data.type !== 'error') {
          $rootScope.auth           = data.user;

          if($zopim.hasOwnProperty('livechat')) {
            if(typeof $rootScope.auth != 'undefined') {
              $zopim.livechat.setName($rootScope.auth.username + '- ('+ $rootScope.auth.first_name + ' '+ $rootScope.auth.last_name +') Mobil Ãœye');
              $zopim.livechat.setEmail($rootScope.auth.email);
              $zopim.livechat.addTags($rootScope.auth.id);

              if($rootScope.auth.user_type_id === 1) {
                $zopim.livechat.addTags('VIP');
              }

              if($rootScope.auth.status === 0) {
                $zopim.livechat.addTags('Hesap:OnaylanmamÄ±ÅŸ');
              }

              if($rootScope.auth.status === 1) {
                $zopim.livechat.addTags('Hesap:Aktif');
              }

              if($rootScope.auth.status === 2) {
                $zopim.livechat.addTags('Hesap:BanlÄ±');
              }
            }

            $zopim.livechat.setLanguage($rootScope.language);
          }

          // if(typeof $rootScope.auth != 'undefined') {
          //   Tawk_API.visitor = {
          //     name: $rootScope.auth.username + '- ('+ $rootScope.auth.first_name + ' '+ $rootScope.auth.last_name +') Mobil Ãœye',
          ...
          // Sports Book Default
          /*$rootScope.sports = {
            //main    : $sce.trustAsResourceUrl($rootScope.sportsURL[$rootScope.currentLang].main + data.sessionId),
            //live    : $sce.trustAsResourceUrl($rootScope.sportsURL[$rootScope.currentLang].live + data.sessionId),
            //virtual : $sce.trustAsResourceUrl(apiURL + '/mobile/sport/redirect?type=virtual&lang=' + $translate.use() + '&t=' + Date.now()),
            //history : $sce.trustAsResourceUrl($rootScope.sportsURL[$rootScope.currentLang].history + data.sessionId)
            main    : $sce.trustAsResourceUrl(apiURL + '/mobile/sport/redirect?type=main&lang=' + $translate.use() + '&t=' + Date.now()),
            live    : $sce.trustAsResourceUrl(apiURL + '/mobile/sport/redirect?type=live&lang=' + $translate.use() + '&t=' + Date.now()),
            virtual : $sce.trustAsResourceUrl(apiURL + '/mobile/sport/redirect?type=virtual&lang=' + $translate.use() + '&t=' + Date.now()),
            history : $sce.trustAsResourceUrl(apiURL + '/mobile/sport/redirect?type=history&lang=' + $translate.use() + '&t=' + Date.now())
          };*/
    ...
    $rootScope.$on('$stateChangeStart', function () {
      $rootScope.getSportLinks();

      $timeout(function() {
        $rootScope.login();
        if($state.current.name !== 'app.live-support') {
          if($zopim.hasOwnProperty('livechat')) {
            if($zopim.livechat.hasOwnProperty('button')) {
    ...
    // Alert Helper
    $rootScope.showAlert = function (title, text, time) {
...
.config(
  ...
  )
  {
    ...
    if(typeof lang === 'undefined') {
      if(baseURL === 'dinamobet.md')
        $httpProvider.defaults.headers.common["User-Language"] = 'ro';
      else
        $httpProvider.defaults.headers.common["User-Language"] = 'tr';
    } else
      $httpProvider.defaults.headers.common["User-Language"] = $cookies.get('NG_TRANSLATE_LANG_KEY');
    ...
    $httpProvider.interceptors.push(function() {
      return {
        'request': function(config) {
          ...
          if(config.url.search('payment') !== -1)
    ...
    function getTimeVersionedUrl(url) {
      ...
      var param = 'timestamp=' + viewVersion; // 4 unique digits every minute
      if (url.indexOf('?') > 0) {
        if (url.indexOf('timestamp=') > 0) return url.replace(/timestamp=[0-9](4)/, param);
        return url + '&' + param;
      }
      return url + '?' + param;
    }
...
.directive('mySelect',['$timeout', function($timeout) {
  return {
    ...
    link: function(scope,element,attrs) {
      $(element).selectOrDie({'customID' : attrs.mySelect});
    }
  };
}]);
